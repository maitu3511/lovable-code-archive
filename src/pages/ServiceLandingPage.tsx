      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="mt-6 flex flex-wrap items-center gap-2 text-xs font-semibold text-[#777]">
            <a href="/" className="hover:text-[#9A7B16] transition-colors">Home</a>
            <ChevronRight className="h-3.5 w-3.5" />
            <a href="/services" className="hover:text-[#9A7B16] transition-colors">Services</a>
            <ChevronRight className="h-3.5 w-3.5" />
        ...
        <div className="mt-14 grid gap-5 md:grid-cols-3">
          {service.sections.map((section, index) => (
            ...
            >
              ...
              <h2 className="mt-5 text-xl font-bold tracking-tight text-[#111] font-heading">{section.heading}</h2>
        ...
        >
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#D4AF37]">Rajkot · Gujarat · Pan-India</p>
              <h2 className="mt-2 text-2xl font-bold font-heading sm:text-3xl">Ready to grow with {service.label}?</h2>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-[#D8D8D8]">Get a practical growth plan built around your goals, audience, budget and conversion journey.</p>
            </div>
            <button
              onClick={() => onOpenConsultation(service.label)}
