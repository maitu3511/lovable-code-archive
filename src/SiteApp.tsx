...
import { getServiceRoute, getServiceCanonical, getServiceSchema, getServiceBreadcrumbSchema } from "./data/serviceRoutes";
...
function AppContent({
  ...
  const handleNavigate = (page: PageType) => {
    ...
    if (isStandaloneRoute) {
      if (page === "services") {
        ...
        navigate({ to: "/", hash: page === "home" ? undefined : page });
      }
  ...
  const handleNavigateService = (serviceId: string) => {
    if (isStandaloneRoute) {
      navigate({ to: "/", hash: "services" });
      return;
    }
    setServiceTargetId(serviceId);
  ...
  return (
    <div className="min-h-screen bg-[#FFFFFF] text-[#111111] font-sans selection:bg-[#D4AF37] selection:text-[#111111] antialiased flex flex-col justify-between">
      ...
      <SEOHead
        ...
        serviceBreadcrumbSchema={serviceRoute ? getServiceBreadcrumbSchema(serviceRoute) : undefined}
      />
